  ...
  const char MISS = '-';
    ...
    BattleshipBoard::BattleshipBoard() : playerTurn(true), gameOver(false) {
    ...
    void BattleshipBoard::loadConfig(const std::string& configFileName){
      ...
      while (std::getline(configFile, line)){
        if (line.find("Boat: ") != std::string::npos) {
          ...
          if (colonPos != std::string::npos && commaPos != std::string::npos) {
              std::string shipName = line.substr(colonPos + 2, commaPos - colonPos - 2);
              int shipSize = std::stoi(line.substr(commaPos + 2));
  
              std::cout << "Ship: " << shipName << ", Size: " << shipSize << std::endl;
          }
        }
          ...
          if (colonPos != std::string::npos && xPosition != std::string::npos) {
              int rows = std::stoi(line.substr(colonPos + 2, xPosition - colonPos - 2));
              int cols = std::stoi(line.substr(xPosition + 1));
  
              
              std::cout << "Board: Rows = " << rows << ", Cols = " << cols << std::endl;
  
              // You can set up your game board dimensions here
          }
      }
      }
       configFile.close();
    ...
    void BattleshipBoard::displayBoards() const {
  ...
  bool BattleshipBoard::isGameOver() const {
  ...
  bool BattleshipBoard::isValidCoordinate(int row, int col, bool isComputer) const {
      ...
      if (isComputer && playerTargetBoard[row][col] != EMPTY) {
          std::cout << "Computer already targeted this location. Try again." << std::endl;
          return false;
  ...
  void BattleshipBoard::autoPlaceShips(std::vector<std::vector<char>>& board) {
